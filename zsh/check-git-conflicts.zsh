...
git -C "$CONFIG_REPO_PATH" fetch

# Get the current branch name
CURRENT_BRANCH=$(git_current_branch)

# Temporary merge to detect conflicts
git -C "$CONFIG_REPO_PATH" checkout -b temp-merge-branch "${CURRENT_BRANCH}"
git -C "$CONFIG_REPO_PATH" merge "${MAIN_BRANCH}" --no-commit --no-ff

# Count the number of conflicts
CONFLICT_COUNT=$(git -C "$CONFIG_REPO_PATH" ls-files -u | wc -l)

# Clean up temporary branch
git -C "$CONFIG_REPO_PATH" merge --abort
git -C "$CONFIG_REPO_PATH" checkout "${CURRENT_BRANCH}"
git -C "$CONFIG_REPO_PATH" branch -D temp-merge-branch
clear
